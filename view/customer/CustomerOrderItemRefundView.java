...
public class CustomerOrderItemRefundView implements View{
    Cresponse m_cresponse;
    CState m_current_state;

    public CustomerOrderItemRefundView(CState current_state) {this.m_current_state = current_state;}
    @Override
    public void Display(){
        ...
        if (order_item == null)
        {
            ...
            System.out.println("Sorry, this item cannot be refunded.");
            m_cresponse = new Cresponse (0);
            return ;
        }
        order_item.set_stat(3);
        try {
        ...
        System.out.println("You will be refunded: "+(order_item.get_amount()*order_item.get_quantity()));

        System.out.println ("Customer Order Item Refund:");
